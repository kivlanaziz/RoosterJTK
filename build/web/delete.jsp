<%-- 
    Document   : delete
    Created on : Jan 1, 2018, 10:08:00 PM
    Author     : kivla
--%>
            ...
            <table>
                <jsp:include page="/Rooster?action=allschedule"/>
                    ...
                    <tr>
                            ...
                            <th> <button class="button button2" onclick="location.href='./delete?action=delete&kd_dosen=<c:out value='${dt.getKD_DOSEN()}' />&kd_matkul=<c:out value='${dt.getKD_MATKUL()}' />&kd_ruangan=<c:out value='${dt.getKD_RUANGAN()}' />&nama_kelas=<c:out value='${dt.getNAMA_KELAS()}' />&prodi=<c:out value='${dt.getPRODI()}' />&kd_jam=<c:out value='${dt.getKD_JAM()}' />'">Delete</a> </th>
                    </tr>
                    </c:forEach>
            </table>
